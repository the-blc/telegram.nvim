...
function M.init()
	state.work = vim.defer_fn(M.process_events, 10)
end
...
function M.emit(event_type, data)
	...
	if state.work then
		...
		vim.notify("Error: Work queue (state.work) is not initialized!", vim.log.levels.ERROR)
	end
end
...
function M.handle_tdlib_update(update)
	...
	elseif update["@type"] == "messages" then
		M.emit(M.events.CHAT_HISTORY, update)
