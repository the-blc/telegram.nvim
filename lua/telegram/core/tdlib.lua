...
function M.init(config)
	...
	state.running = true
	state.updates_timer = vim.defer_fn(M.update_loop, 10)
...
function M.update_loop()
	...
	local update = nil
	ok, update = pcall(json.decode, ffi.string(response))
	if not ok then
...
function M.handle_update(update)
	...
	--     logging events in order to understand what kind of data telegram sends
	...
	if handlers then
		...
		Core.events.handle_tdlib_update(update)
	end
end
