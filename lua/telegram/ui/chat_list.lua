		...
		if chat.last_message.sender_id then
			if chat.last_message.sender_id["@type"] == "messageSenderUser" then
				...
				if user then
					...
					if
						chat.last_message
						and chat.last_message.sender_id
						and chat.last_message.sender_id["@type"] == "messageSenderUser"
					then
						Users.add_user(chat.last_message.sender_id.user_id, {})
						Core.tdlib.get_user(chat.last_message.sender_id.user_id)
					end
				end
			end
		end
...
local function render_chat(chat)
	...
	if state.selected_chat then
		local chat_line = state.chat_line_map[state.selected_chat.id]
		if chat_line then
			table.insert(state.all_highlights, {
	...
	local cursor_line = api.nvim_win_get_cursor(state.window)[1] - 1
	if state.line_to_chat_map[cursor_line] then
		table.insert(state.all_highlights, {
			group = "TelegramChatCursor",
			line = cursor_line,
			col_start = 0,
			col_end = -1,
		})
...
local function update_display()
	...
	local chat_line_map = {}
	local line_to_chat_map = {}
	-- Render each chat
	for _, chat in ipairs(sorted_chats) do
		chat_line_map[chat.chat_id] = current_line
		line_to_chat_map[current_line] = chat.chat_id
	...
	state.chat_line_map = chat_line_map
	state.line_to_chat_map = line_to_chat_map
end
...
function M.init()
	...
	vim.keymap.set("n", "j", function()
		-- First perform normal j movement
		vim.cmd("normal! j")

		-- Then update selected chat based on new cursor position
		local chat = M.get_chat_at_cursor()
		if chat then
			state.selected_chat = chat
			update_display()
			Core.events.emit(Core.events.events.UI_CHAT_SELECTED, chat)
		end
	end, {
		buffer = state.buffer,
		silent = true,
	...
	vim.keymap.set("n", "<CR>", function()
		...
		if
			state.folder_section
			and cursor >= state.folder_section.start
			and cursor <= state.folder_section.folders_end
		then
	...
	Core.events.on(Core.events.events.CHAT_LAST_MESSAGE_UPDATED, function(chat)
		...
		if not state.chats[chat.chat_id] then
			Core.tdlib.get_chat(chat.chat_id)
		else
	...
	Core.events.on(Core.events.events.CHAT, function(chat)
		-- Only store chat if we have a position for it in the current list
		-- vim.notify(vim.inspect(tdlib.get_chat(chat.chat_id)), vim.log.levels.INFO)
		-- state.chats[chat.chat_id] = tdlib.get_chat(chat.chat_id)
		...
		if chat.photo then
			if
				not chat.photo.small["local"].is_downloading_completed and chat.photo.small["local"].can_be_downloaded
			then
				Core.tdlib.download_file(chat.photo.small.id)
			end
		end
		update_display()
	end)

	Core.events.on(Core.events.events.CHAT_DELETED, function(chat_id)
	...
	Core.events.on(Core.events.events.CHAT_POSITION_UPDATED, function(data)
		-- Only update position if it matches our current list type
		if
			data.position.list["@type"] == "chatListMain" and state.current_list_type == "main"
			or data.position.list["@type"] == "chatListArchive" and state.current_list_type == "archive"
			or data.position.list["@type"] == "chatListFilter"
				and state.current_list_type == "filter"
				and data.position.list.chat_filter_id == state.current_filter_id
		then
	...
	Core.events.on(Core.events.events.CHAT_FOLDERS_UPDATED, function(data)
	...
	Core.events.on(Core.events.events.CHATS, function(data)
		for _, chat_id in ipairs(data.chat_ids) do
			if not state.chats[chat_id] then
				Core.tdlib.get_chat(chat_id)
			end
		end
	end)

	Core.events.on(Core.events.events.CHAT_LIST_UPDATED, function(data)
		...
		elseif data.chat_list["@type"] == "chatListArchive" and state.current_list_type == "archive" then
			matches = true
		elseif
			data.chat_list["@type"] == "chatListFilter"
			and state.current_list_type == "filter"
			and data.chat_list.chat_filter_id == state.current_filter_id
		then
			matches = true
		end
...
function M.get_chat_at_cursor()
	local cursor_pos = api.nvim_win_get_cursor(state.window)
	local line = cursor_pos[1] - 1 -- 0-indexed

	-- Check if the cursor is in a chat section
	if state.folder_section and line <= state.folder_section.folders_end then
		-- Cursor is in the folder section, not on a chat
		return nil
	end

	if line >= state.chat_start then
		line = line - state.chat_start
		-- Find the nearest chat line above the cursor
		for i = line, 0, -1 do
			local chat_id = state.line_to_chat_map[i]
			if chat_id then
...
function M.switch_chat_list(list_type, filter_id)
	vim.notify(
		"Switching chat list to: " .. list_type .. (filter_id and (" (filter " .. filter_id .. ")") or ""),
		vim.log.levels.INFO
	)
	...
	if list_type == "main" then
		chat_list = Core.tdlib.get_chat_list_main()
	elseif list_type == "archive" then
		chat_list = Core.tdlib.get_chat_list_archive()
	elseif list_type == "filter" then
		chat_list = Core.tdlib.get_chat_list_filter(filter_id)
	end

	-- Load chats for the selected list
	local request_id = Core.tdlib.load_chats(chat_list, 100)
	vim.notify(
		"Loading chats for " .. list_type .. " list with request ID: " .. tostring(request_id),
		vim.log.levels.INFO
	)
	...
	vim.defer_fn(function()
		if state.loading then
			...
			update_display()
			vim.notify(
				"Chat list load timeout - displaying available chats. Try reloading with 'r'",
				vim.log.levels.WARN
			)
		end
	end, 5000)
...
function M.select_next_chat()
	...
	-- Move cursor to the chat line
	if next_line then
		api.nvim_win_set_cursor(state.window, { next_line + 1, 0 })
		local chat_id = state.line_to_chat_map[next_line]
		if chat_id then
...
function M.select_previous_chat()
	...
	if state.selected_chat then
		for i, chat in ipairs(sorted_chats) do
			if chat.id == state.selected_chat.chat_id then
				current_index = i
				break
			end
		end
