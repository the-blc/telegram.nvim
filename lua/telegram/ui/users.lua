...
function M.init()
	...
	Core.events.on(Core.events.events.USER_PROFILE_UPDATED, function(user)
		if not user then
			return
		end
...
function M.get_profile_photo(user_id)
	...
	if user.profile_photo.small then
		...
		if not user.profile_photo.small["local"].is_downloading_active then
			Core.tdlib.download_file(user.profile_photo.id, 1)
		end
