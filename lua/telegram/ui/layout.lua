...
function M.create(callback)
	...
	api.nvim_create_autocmd("WinClosed", {
		...
		callback = function(args)
			...
			if
				win_id
				and (
					win_id == state.windows.chat_list
					or win_id == state.windows.messages
					or win_id == state.windows.input
				)
			then
				M.destroy()
			end
	...
	Core.events.emit(Core.events.events.UI_LAYOUT_CHANGED, {
...
function M.destroy()
	pcall(function()
		...
		-- Emit layout changed event
		Core.events.emit(Core.events.events.UI_LAYOUT_CHANGED, nil)
	end)
end
