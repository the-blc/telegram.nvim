...
local function update_display()
	...
	local sorted_messages = state.messages

	-- Render each message
	for i = 1, #sorted_messages do
		local message = sorted_messages[#sorted_messages + 1 - i]
...
function M.init()
	...
	api.nvim_buf_set_keymap(state.buffer, "n", "<C-u>", "", {
		callback = function()
			api.nvim_win_call(state.window, function()
	...
	Core.events.on(Core.events.events.MESSAGE_RECEIVED, function(message)
		if state.current_chat and message.chat_id == state.current_chat.id then
			if state.messages then
				table.insert(state.messages, 0, message)
	...
	Core.events.on(Core.events.events.CHAT_HISTORY, function(message)
		if state.messages then
			state.total_count = message.total_count
			state.messages = message.messages
			update_display()
		end
	end)

	Core.events.on(Core.events.events.MESSAGE_EDITED, function(update)
		if state.current_chat and update.chat_id == state.current_chat.id then
			if state.messages[update.message_id] then
				state.messages[update.message_id] = vim.tbl_extend("force", state.messages[update.message_id], update)
				update_display()
			end
		end
	end)
...
function M.load_chat(chat)
	...
	pcall(function()
		Core.tdlib.get_chat_history(chat.chat_id, 0, 50)
	end)
...
function M.load_more_messages()
	if not state.current_chat or state.loading then
		return
	end
	...
	Core.tdlib.get_chat_history(state.current_chat.id, state.last_message_id, 50)
...
function M.handle_media(message)
	...
	if content_type == "messagePhoto" then
		...
		if not state.media_previews[file_id] then
			Core.tdlib.download_file(file_id, 1)
		end
	end
end
