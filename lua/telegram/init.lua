...
function M.start()
	...
	Core.tdlib.on("updateAuthorizationState", function(update)
...
M.open_chat = function(chat_id)
	...
	UI.chat_list.open_chat(chat_id)
end

M.send_message = function(text)
	if not state.initialized or not state.current_chat then
		return
	end
	Core.tdlib.send_message(state.current_chat, text)
end
...
M.send_file = function(path)
	...
	Core.tdlib.send_file(state.current_chat, path)
end
